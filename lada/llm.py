...
class GeminiClient:
        ...
        *,
        ...
        try:
            return self._walk("image", self._candidates("image", model), attempt,
                              retries=1)
        except (QuotaError, ModelUnavailable) as exc:
            return self._fallback_image(prompt, out_path, aspect_ratio,
                                        context, exc)

    def _fallback_image(self, prompt: str, out_path: Path, aspect_ratio: str,
                        context: CallContext | None, cause: Exception) -> Path:
        """Second-provider attempt after Gemini runs out of image quota."""
        ...
        try:
            result = providers.generate_image(
                prompt, out_path, aspect_ratio=aspect_ratio,
                context=context or self.context)
        except Exception as fallback_exc:
            self._note(f"OpenAI image fallback failed: "
                       f"{security.redact(fallback_exc)[:180]}")
            raise cause from None
        self._note(f"Image generated by {result.provider}/{result.model} - "
                   "the Gemini image quota is exhausted.")
        return result.path
    ...
    def synthesize_speech(
        ...
        *,
        ...
        try:
            return self._walk("tts", self._candidates("tts", model), attempt,
                              retries=1)
        except (QuotaError, ModelUnavailable) as exc:
            return self._fallback_speech(clean, out_path, voice, style, accent,
                                         context, exc)

    def _fallback_speech(self, script: str, out_path: Path, voice: str,
                         style: str, accent: str,
                         context: CallContext | None, cause: Exception) -> Path:
        """Second-provider attempt after Gemini runs out of TTS quota."""
        from . import providers

        if not providers.is_configured():
            raise cause
        try:
            result = providers.synthesize_speech(
                script, out_path, voice=voice, style=style, accent=accent,
                context=context or self.context)
        except Exception as fallback_exc:
            self._note(f"OpenAI narration fallback failed: "
                       f"{security.redact(fallback_exc)[:180]}")
            raise cause from None
        self._note(f"Narration produced by {result.provider}/{result.model} - "
                   "the Gemini TTS quota is exhausted.")
        return result.path
